...
extern uint32_t _estack , _etext ,_sdata , _edata , _sbss , _ebss , _eheap , _sidata ;
extern int main(void);


void System_Intailization              (void);
...
uint32_t *const MSP_Value = &_estack ;

/* Intializing vector table addresses */ 
...
void Reset_Handler(void)
{
    ...
    uint32_t BSS_Size = 0 ;
    uint32_t *Flash_Data_Start = NULL ;
    uint32_t *RAM_Data_Start = NULL ;
    uint32_t *BSS_Start = NULL ;

    /* Copy data segment from Flash to Ram */
    RAM_Data_Size = &_edata - &_sdata ;
    Flash_Data_Start = &_sidata ;
    RAM_Data_Start = &_sdata ;

    for(uint32_t counter = 0 ; counter < RAM_Data_Size ; counter++ )
    {
        *RAM_Data_Start++ = *Flash_Data_Start++ ;
    }

    /* Intializing .bss segment with zero */
    BSS_Size = &_ebss - &_sbss ; 
    BSS_Start = &_sbss ;

    for(uint32_t counter = 0 ; counter < bss_Size ; counter++ )
    {
        *BSS_Start++ = 0 ;
    }

    /* Call the system intialization function */
    System_Intailization();
